<?php
// Cek session agar tidak muncul Notice session_start()
...
// Pastikan id_user terdeteksi baik dari SESSION maupun GET
...
<?php if ($tab === 'liked'): ?>
    ...
    <div class="grid-photos">
        <?php
        $q = "SELECT f.* FROM like_foto l JOIN foto f ON l.id_foto = f.id_foto WHERE l.id_user = ? ORDER BY l.tanggal_like DESC";
        ...
        ?>
            <div class="grid-item">
                <img src="../../backend/uploads/<?= htmlspecialchars($row['lokasi_file']) ?>" alt="Liked Foto">
            </div>
        <?php 
            endwhile;
        else:
            echo "<p style='color: #767676;'>Belum ada foto yang disukai.</p>";
        endif;
        $stmt->close();
        ?>
    </div>
...
<?php elseif ($tab === 'boards'): ?>
    ...
    <div class="boards-grid">
            ...
            <a href="detail_board.php?id_album=<?= $album['id_album']; ?>" class="board-card-container">
                <div class="board-cover">
                    <?php if (!empty($album['foto_sampul'])): ?>
                        <img src="../../backend/uploads/<?= htmlspecialchars($album['foto_sampul']); ?>" alt="Sampul Board">
                    <?php else: ?>
        ...
        <?php 
            ...
            // JIKA BELUM MEMILIKI ALBUM, TAMPILKAN LANGSUNG FOTO UNGGAHAN USER
            $qFotoUser = "SELECT * FROM foto WHERE id_user = ? ORDER BY tanggal_ungahan DESC";
            ...
            ?>
                <div class="grid-photos" style="grid-column: 1/-1;">
                    <?php while ($fu = $resFU->fetch_assoc()): ?>
                        <div class="grid-item">
                            <img src="../../backend/uploads/<?= htmlspecialchars($fu['lokasi_file']) ?>" alt="Foto User">
                        </div>
                    <?php endwhile; ?>
                </div>
            <?php
            else:
                echo "<p style='color: #767676; grid-column: 1/-1;'>Kamu belum punya board atau foto.</p>";
            endif;
            $stmtFU->close();
        endif;
        $stmtA->close();
...
<?php else: ?>
    ...
    <div style="margin-top: 20px;">
        <p style="color: #767676;">Belum ada collage.</p>
    </div>
<?php endif; ?>